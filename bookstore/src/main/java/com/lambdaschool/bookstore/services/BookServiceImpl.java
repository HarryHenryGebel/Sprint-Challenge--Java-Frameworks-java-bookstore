...
public class BookServiceImpl implements BookService {
  ...
  public Book save(Book book) {
    ...
    if (book.getSection() != null) {
      newBook.setSection(
        sectionService.findSectionById(book.getSection().getSectionid())
      );
    }

    newBook.getWrotes().clear();
    for (Wrote w : book.getWrotes()) {
      Author addAuthor = authorrepos
      ...
      newBook.getWrotes().add(new Wrote(addAuthor, newBook));
    }
  ...
  public Book update(Book book, long id) {
    ...
    if (book.getSection() != null) {
      currentBook.setSection(
        sectionService.findSectionById(book.getSection().getSectionid())
      );
    }

    if (book.getWrotes().size() > 0) {
      currentBook.getWrotes().clear();
      for (Wrote w : book.getWrotes()) {
        Author addAuthor = authorrepos
          .findById(w.getAuthor().getAuthorid())
          .orElseThrow(
        ...
        currentBook.getWrotes().add(new Wrote(addAuthor, currentBook));
      }
    }
